...
heap_pages_expand_sorted_to(rb_objspace_t *objspace, size_t next_length)
{
    ...
    size_t global_size = size_mul_or_raise(next_length_global, sizeof(struct heap_page *), rb_eRuntimeError);
...
newobj_init(VALUE klass, VALUE flags, int wb_protected, rb_objspace_t *objspace, VALUE obj)
{
    ...
    *((VALUE *)&p->as.basic.klass) = klass;
	...
	VM_ASSERT(heap_locked(objspace));
...
newobj_of(rb_ractor_t *cr, VALUE klass, VALUE flags, VALUE v1, VALUE v2, VALUE v3, int wb_protected, size_t alloc_size)
{
    ...
    rb_ractor_newobj_cache_t *cache;
    bool borrowing = !!(objspace->local_data.alloc_target_ractor);
    if (borrowing) {
	objspace = objspace->local_data.alloc_target_ractor->local_objspace;
	cache = &objspace->local_data.ractor->newobj_borrowing_cache;
    }
    else {
...
PUREFUNC(static inline int pointer_is_on_page(register uintptr_t p, register struct heap_page *page);)
static inline int
...
PUREFUNC(static inline int heap_page_possible(rb_objspace_t *objspace, register uintptr_t p, uintptr_t lomem, uintptr_t himem);)
static inline int
...
PUREFUNC(static inline int is_pointer_to_local_heap(rb_objspace_t *objspace, const void *ptr);)
static inline int
...
PUREFUNC(static inline int is_pointer_to_global_heap(rb_objspace_t *objspace, const void *ptr);)
static inline int
...
obj_free(rb_objspace_t *objspace, VALUE obj)
{
    ...
    if (RVALUE_LOCAL_IMMUNE(obj)) {
	VM_ASSERT(!using_local_limits(objspace));
	CLEAR_IN_BITMAP(GET_HEAP_LOCAL_IMMUNE_BITS(obj), obj);
    }
...
gc_sweep_start(rb_objspace_t *objspace)
{
    ...
    rb_gc_ractor_newobj_cache_clear(&objspace->local_data.ractor->newobj_cache);
    rb_gc_ractor_newobj_cache_clear(&objspace->local_data.ractor->newobj_borrowing_cache);
}
...
#define MARK_CHECKPOINT(category) do { \
    ...
    rb_ractor_related_objects_mark(objspace->local_data.ractor);
    mark_contained_ractor_tbl(objspace);
    ...
    rb_native_mutex_lock(&objspace->local_data.obj_id_lock);
    mark_tbl_no_pin(objspace, objspace->local_data.obj_to_id_tbl); /* Only mark ids */
    rb_native_mutex_unlock(&objspace->local_data.obj_id_lock);
                              ...
                              struct verify_internal_consistency_struct *data)
{
    ...
    for (obj = (VALUE)page_start; obj != (VALUE)page_end; obj += stride) {
		...
		if (RVALUE_MARKED(obj)) {
		    ...
		    VM_ASSERT(RVALUE_LOCAL_IMMUNE(obj));
		}
	    }
...
gc_update_weak_references(rb_objspace_t *objspace)
{
    ...
    rb_darray_foreach(objspace->weak_references, i, ptr_ptr) {
        ...
        if (!RVALUE_MARKED(obj) && !(using_local_limits(objspace) && RVALUE_LOCAL_IMMUNE(obj))) {
            **ptr_ptr = Qundef;
        }
        else {
...
rb_gc_register_mark_object(VALUE obj)
{
	...
	WITH_OBJSPACE_OF_VALUE_ENTER(obj, objspace);
	{
	    ret = rb_run_with_redirected_allocation(objspace->local_data.ractor, register_mark_object_no_redirection, obj);
	}
	WITH_OBJSPACE_OF_VALUE_LEAVE(objspace);
    }
    return ret;
...
gc_is_moveable_obj(rb_objspace_t *objspace, VALUE obj)
{
    ...
    if (using_local_limits(objspace) && (FL_TEST_RAW(obj, FL_SHAREABLE) || RVALUE_LOCAL_IMMUNE(obj))) {
	return FALSE;
    }
...
gc_move(rb_objspace_t *objspace, VALUE scan, VALUE free, size_t src_slot_size, size_t slot_size)
{
    ...
    VM_ASSERT(!RVALUE_LOCAL_IMMUNE((VALUE)src));
...
gc_update_references(rb_objspace_t *objspace)
{
    ...
    rb_ractor_update_references(objspace->local_data.ractor);
...
obj_info(VALUE obj)
{
	...
	const char *ret = rb_raw_obj_info(buff, OBJ_INFO_BUFFERS_SIZE, obj);
    }
    RB_VM_LOCK_LEAVE();
    return ret;
}
...
rb_gcdebug_print_obj_condition(VALUE obj)
{
    ...
    fprintf(stderr, "local-immune?   : %s\n", RVALUE_LOCAL_IMMUNE(obj) ? "false" : "true");
...
{
    return RVALUE_LOCAL_IMMUNE(obj);
}
