...
static void
wmap_entry_free(struct weakmap_entry *entry)
{
    if (entry) {
	rb_remove_from_external_weak_tables(&entry->key);
	rb_remove_from_external_weak_tables(&entry->val);
    }
    ruby_sized_xfree(entry, sizeof(struct weakmap_entry));
}

static void
wkmap_entry_free(struct weakmap_entry *entry)
{
    if (entry) {
	rb_remove_from_external_weak_tables(&entry->key);
    }
    ruby_sized_xfree(entry, sizeof(VALUE));
}

static void
wmap_remove_from_external_tables(struct weakmap_entry *dead_entry, bool remove_key, bool remove_val)
{
    if (dead_entry) {
	if (remove_key) rb_remove_from_external_weak_tables(&dead_entry->key);
	if (remove_val) rb_remove_from_external_weak_tables(&dead_entry->val);
    }
}
...
wmap_foreach_i(st_data_t key, st_data_t val, st_data_t arg)
{
	...
	wmap_entry_free(data->dead_entry);
...
wmap_foreach(struct weakmap *w, int (*func)(struct weakmap_entry *, st_data_t), st_data_t arg)
{
    ...
    wmap_entry_free(foreach_data.dead_entry);
}
...
wmap_lookup(VALUE self, VALUE key)
{
    ...
    if (SPECIAL_CONST_P(*(VALUE *)data)) return *(VALUE *)data;
...
wkmap_delete(VALUE self, VALUE key)
{
	...
	rb_remove_from_external_weak_tables(orig_key_data);
