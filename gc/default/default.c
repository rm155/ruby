...
typedef struct {
    ...
    double sharedobject_limit_factor;
...
typedef struct rb_objspace {
    ...
    struct rb_borrowing_location_lock location_locks[HEAP_COUNT];
...
total_allocated_objects(rb_objspace_t *objspace)
{
	...
	heap->total_allocated_objects += heap->newly_created_by_borrowing_count;
	heap->newly_created_by_borrowing_count = 0;
    }
...
static void gc_rest_global(rb_objspace_t *objspace);
...
static inline int
RVALUE_MUTABLE_SHAREABLE_PERMISSION(rb_objspace_t *objspace, VALUE obj)
{
    check_rvalue_consistency(objspace, obj);
...
void
rb_gc_impl_permit_mutable_shareable_direct(VALUE obj)
{
    //TODO enforce assertion
    //VM_ASSERT(RVALUE_WB_UNPROTECTED_BITMAP(obj) == 0);
...
void *
rb_gc_impl_objspace_of_value(VALUE obj)
{
    return (void *)RUBY_ATOMIC_PTR_LOAD(GET_HEAP_PAGE(obj)->objspace);
}

void *
rb_gc_impl_ractor_of_value(VALUE obj)
{
    return (void *)RUBY_ATOMIC_PTR_LOAD(GET_HEAP_PAGE(obj)->ractor);
}

void *
rb_gc_impl_ractor_of_objspace(void *objspace_ptr)
{
    rb_objspace_t *objspace = objspace_ptr;
    return (void *)objspace->ractor;
}

void *
rb_gc_impl_local_gate_of_objspace(void *objspace_ptr)
{
    rb_objspace_t *objspace = objspace_ptr;
    return (void *)objspace->local_gate;
}
...
check_rvalue_consistency_force(rb_objspace_t *objspace, const VALUE obj, int terminate)
{
	...
	{
	    err = check_rvalue_consistency_force(obj_objspace, obj, terminate);
	}
	WITH_OBJSPACE_OF_VALUE_LEAVE(obj_objspace);
	return err;
    }
...
void
rb_gc_impl_gc_deactivate_prepare(void *objspace_ptr)
{
    rb_objspace_t *objspace = objspace_ptr;
...
const struct st_hash_type object_id_hash_type = {
    object_id_cmp,
    object_id_hash,
};
...
heap_pages_free_unused_pages(rb_objspace_t *objspace)
{
    if (objspace->empty_pages != NULL && heap_pages_freeable_pages > 0) {
        ...
        for (i = j = 0; i < rb_darray_size(objspace->heap_pages.sorted); i++) {
            ...
            if (heap_page_in_objspace_empty_pages_pool(objspace, page) && heap_pages_freeable_pages > 0) {
		page->unlinked = true;
            }
            else {
                if (heap_page_in_objspace_empty_pages_pool(objspace, page)) {
...
static void
heap_append_added_page(rb_objspace_t *objspace, rb_heap_t *heap, struct heap_page *page)
{
...
heap_add_page(rb_objspace_t *objspace, rb_heap_t *heap, struct heap_page *page, bool just_allocated)
{
    ...
    heap_append_added_page(objspace, heap, page);
}
...
heap_page_allocate_and_initialize(rb_objspace_t *objspace, rb_heap_t *heap)
{
    ...
    if (page != NULL) {
        heap_add_page(objspace, heap, page, allocated);
...
static void
heap_prepare(rb_objspace_t *objspace, rb_heap_t *heap, bool borrowing)
{
...
static struct heap_page *
heap_next_free_page(rb_objspace_t *objspace, rb_heap_t *heap, bool borrowing)
{
    struct heap_page *page;

    if (heap->free_pages == NULL) {
        heap_prepare(objspace, heap, borrowing);
    }
...
static void gc_ractor_newobj_heap_cache_clear(void *c);
static VALUE
newobj_borrowing_cache_miss(rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx, bool need_new_borrowing_page)
{
	...
	if (obj == Qfalse) {
	    ...
	    struct heap_page *page = heap_next_free_page(objspace, heap, true);
	    if (need_new_borrowing_page)
	    {
		rb_ractor_newobj_heap_cache_t *heap_cache = &cache->heap_caches[heap_idx];
		gc_ractor_newobj_heap_cache_clear(heap_cache);
	    }

	    ractor_cache_set_page(objspace, cache, heap_idx, page);

	    // Retry allocation after moving to new page
	    obj = ractor_cache_allocate_slot(objspace, cache, heap_idx, true);
	}
    }
    OBJSPACE_LOCK_LEAVE(objspace);
...
static VALUE
newobj_alloc_borrowing(rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx)
{
    ...
    borrowable_page_locked = !rb_native_mutex_trylock(&objspace->location_locks[heap_idx].page_lock);
    if (borrowable_page_locked) {
	objspace->location_locks[heap_idx].page_lock_owner = cr;
    }

    bool need_new_borrowing_page = !borrowable_page_locked && objspace->location_locks[heap_idx].page_recently_locked;
    ...
    if (RB_UNLIKELY(obj == Qfalse)) {
	obj = newobj_borrowing_cache_miss(objspace, cache, heap_idx, need_new_borrowing_page);
    }

    RUBY_ATOMIC_SIZE_ADD(heaps[heap_idx].newly_created_by_borrowing_count, 1);

    if (borrowable_page_locked) {
	objspace->location_locks[heap_idx].page_lock_owner = NULL;
	rb_native_mutex_unlock(&objspace->location_locks[heap_idx].page_lock);
    }
    objspace->location_locks[heap_idx].page_recently_locked = false;

    return obj;
}

NOINLINE(static VALUE newobj_cache_miss(rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx));

static VALUE
newobj_cache_miss(rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx)
{
    rb_heap_t *heap = &heaps[heap_idx];
    VALUE obj = Qfalse;
    ...
    OBJSPACE_LOCK_ENTER(objspace);
    {
        if (is_incremental_marking(objspace)) {
            ...
            obj = ractor_cache_allocate_slot(objspace, cache, heap_idx, false);
        }

        if (obj == Qfalse) {
            // Get next free page (possibly running GC)
            struct heap_page *page = heap_next_free_page(objspace, heap, false);
            ractor_cache_set_page(objspace, cache, heap_idx, page);
...
static VALUE
newobj_alloc(rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx)
{
    VALUE obj = ractor_cache_allocate_slot(objspace, cache, heap_idx, false);

    if (RB_UNLIKELY(obj == Qfalse)) {
        obj = newobj_cache_miss(objspace, cache, heap_idx);
    }
...
ALWAYS_INLINE(static VALUE newobj_slowpath(VALUE klass, VALUE flags, rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, int wb_protected, size_t heap_idx, bool borrowing));

static inline VALUE
newobj_slowpath(VALUE klass, VALUE flags, rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, int wb_protected, size_t heap_idx, bool borrowing)
{
    VALUE obj;
    unsigned int lev;
    ...
    obj = UNLIKELY(borrowing) ? newobj_alloc_borrowing(objspace, cache, heap_idx) : newobj_alloc(objspace, cache, heap_idx);
...
NOINLINE(static VALUE newobj_slowpath_wb_protected(VALUE klass, VALUE flags,
                                                   rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx, bool borrowing));
NOINLINE(static VALUE newobj_slowpath_wb_unprotected(VALUE klass, VALUE flags,
                                                     rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx, bool borrowing));

static VALUE
newobj_slowpath_wb_protected(VALUE klass, VALUE flags, rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx, bool borrowing)
{
    return newobj_slowpath(klass, flags, objspace, cache, TRUE, heap_idx, borrowing);
}

static VALUE
newobj_slowpath_wb_unprotected(VALUE klass, VALUE flags, rb_objspace_t *objspace, rb_ractor_newobj_cache_t *cache, size_t heap_idx, bool borrowing)
{
...
rb_gc_impl_new_obj(void *objspace_ptr, void *cache_ptr, VALUE klass, VALUE flags, VALUE v1, VALUE v2, VALUE v3, bool wb_protected, size_t alloc_size, bool borrowing)
{
    ...
    if (!RB_UNLIKELY(during_gc || ruby_gc_stressful) &&
        ...
        obj = wb_protected ?
          newobj_slowpath_wb_protected(klass, flags, objspace, cache, heap_idx, borrowing) :
...
PUREFUNC(static inline bool heap_page_possible(register uintptr_t p, uintptr_t lomem, uintptr_t himem);)
static inline bool
heap_page_possible(register uintptr_t p, uintptr_t lomem, uintptr_t himem)
{
...
PUREFUNC(static inline bool is_pointer_to_local_heap(rb_objspace_t *objspace, const void *ptr);)
static inline bool
is_pointer_to_local_heap(rb_objspace_t *objspace, const void *ptr)
{
    register uintptr_t p = (uintptr_t)ptr;
    register struct heap_page *page = NULL;

    if (during_gc) {
	if (heap_page_possible(p, heap_pages_lomem, heap_pages_himem) == TRUE) {
...
PUREFUNC(static inline bool is_pointer_to_global_heap(rb_objspace_t *objspace, const void *ptr);)
static inline bool
...
{
    if (is_pointer_to_local_heap(objspace, ptr)) {
	return TRUE;
    }
    else if (!objspace->local_gate->belong_to_single_main_ractor) {
	int ret;
	SUSPEND_OBJSPACE_LOCK_BEGIN(objspace);
	{
	    ret = is_pointer_to_global_heap(objspace, ptr);
	}
...
struct each_obj_data {
    ...
    bool using_borrowable_page[HEAP_COUNT];
};
...
objspace_each_objects_ensure(VALUE arg)
{
	...
	if (data->using_borrowable_page[i]) {
	    rb_ractor_t *cr = GET_RACTOR();
	    unlock_own_borrowable_page(cr, &objspace->location_locks[i]);
	}
    }
...
static struct heap_page *
current_borrowable_page(rb_ractor_t *r, int heap_idx)
{
    rb_ractor_newobj_cache_t *cache = (rb_ractor_newobj_cache_t *)r->newobj_borrowing_cache;
    return cache->heap_caches[heap_idx].using_page;
}

static int
get_heap_idx(rb_objspace_t *objspace, rb_heap_t *heap)
{
    for (int i = 0; i < HEAP_COUNT; i++) {
	if(&heaps[i] == heap) {
	    return i;
...
#define POSSIBLE_USAGE_OF_BORROWING_PAGE_BEGIN(objspace, page) { \
    ...
    int _heap_idx = get_heap_idx(objspace, page->heap); \
    bool _using_borrowable_page = false; \
    if (!objspace->local_gate->freeing_all) { \
	rb_borrowing_sync_lock(_cr); \
	if (current_borrowable_page(_cr, _heap_idx) == page) { \
	    lock_own_borrowable_page(_cr, &objspace->location_locks[_heap_idx]); \
...
#define POSSIBLE_USAGE_OF_BORROWING_PAGE_END() \
	...
	unlock_own_borrowable_page(_cr, &objspace->location_locks[_heap_idx]); \
    } \
}
...
objspace_each_objects_try(VALUE arg)
{
    ...
    for (int heap_idx = 0; heap_idx < HEAP_COUNT; heap_idx++) {
        rb_heap_t *heap = &heaps[heap_idx];
        size_t pages_count = data->pages_counts[heap_idx];
        struct heap_page **pages = data->pages[heap_idx];
	    ...
	    if (current_borrowable_page(r, heap_idx) == page) {
		lock_own_borrowable_page(r, &objspace->location_locks[heap_idx]);
		data->using_borrowable_page[heap_idx] = true;
	    }
	    ...
	    if (data->using_borrowable_page[heap_idx]) {
		unlock_own_borrowable_page(r, &objspace->location_locks[heap_idx]);
		data->using_borrowable_page[heap_idx] = false;
	    }
...
rb_gc_impl_shutdown_free_objects(void *objspace_ptr)
{
    ...
    objspace->local_gate->freeing_all = true;

    for (size_t i = 0; i < rb_darray_size(objspace->heap_pages.sorted); i++) {
        struct heap_page *page = rb_darray_get(objspace->heap_pages.sorted, i);

	POSSIBLE_USAGE_OF_BORROWING_PAGE_BEGIN(objspace, page);
	...
	POSSIBLE_USAGE_OF_BORROWING_PAGE_END();
    }
...
gc_sweep_plane(rb_objspace_t *objspace, rb_heap_t *heap, uintptr_t p, bits_t bitset, struct gc_sweep_context *ctx)
{
    ...
    do {
		...
		objspace->local_gate->shareable_object_estimate++;
		if (is_full_marking(objspace) && RVALUE_OLD_P(objspace, vp)) {
		    objspace->rgengc.old_objects++;
		}
		p += slot_size;
		bitset >>= slot_bits;
...
gc_sweep_page(rb_objspace_t *objspace, rb_heap_t *heap, struct gc_sweep_context *ctx)
{
    ...
    bool local_immunity_active = using_local_limits(objspace);
...
static void
gc_ractor_newobj_heap_cache_clear(void *c)
{
    rb_ractor_newobj_heap_cache_t *cache = (rb_ractor_newobj_heap_cache_t *)c;
...
gc_ractor_newobj_cache_clear(void *c, void *data)
{
	...
	gc_ractor_newobj_heap_cache_clear(cache);
    }
}
...
gc_sweep_finish(rb_objspace_t *objspace)
{
    ...
    if (is_full_marking(objspace)) {
	objspace->local_gate->prev_shareable_object_estimate = objspace->local_gate->shareable_object_estimate;
    }
...
gc_mark_in_range(rb_objspace_t *objspace, VALUE obj)
{
    ...
    if (!ruby_single_main_objspace && FL_TEST_RAW(obj, FL_SHAREABLE)) {
	if (objspace->flags.during_global_gc) {
	    objspace->local_gate->global_shareable_object_estimate++;
	}
	else if (is_full_marking(objspace)){
	    objspace->local_gate->shareable_object_estimate++;
	}
...
gc_mark(rb_objspace_t *objspace, VALUE obj)
{
    ...
    if (using_local_limits(objspace) && !in_local_marking_range(objspace, obj)) return;
...
void
rb_gc_impl_stack_location_mark_maybe(void *objspace_ptr, VALUE obj)
{
    rb_objspace_t *objspace = objspace_ptr;
...
gc_update_weak_references(rb_objspace_t *objspace)
{
    ...
    rb_darray_foreach(objspace->weak_references, i, ptr_ptr) {
        ...
        if (!RVALUE_MARKED(objspace, obj) && !(using_local_limits(objspace) && FL_TEST_RAW(obj, FL_SHAREABLE))) {
            **ptr_ptr = Qundef;
        }
        else {
...
gc_marks_prepare(rb_objspace_t *objspace, int full_mark)
{
	...
	objspace->local_gate->shareable_object_estimate = 0;
	objspace->local_gate->global_shareable_object_estimate = 0;
...
#if RGENGC_PROFILE > 0
static void
gc_record_old_objects(rb_objspace_t *objspace)
{
...
#if RGENGC_PROFILE > 0
    gc_record_old_objects(objspace);
#endif
...
static void
gc_marks_prepare_full(rb_objspace_t *objspace)
{
    gc_marks_prepare(objspace, true);
}

static void
gc_marks_start_full(rb_objspace_t *objspace)
{
    gc_marks_start(objspace, true);
}

static void
gc_marks_global(rb_objspace_t *objspace)
{
    rb_vm_t *vm = GET_VM();

    rb_objspace_gate_t *local_gate = objspace->local_gate;

    global_gc_for_each_objspace(vm, local_gate, gc_prof_mark_timer_start);
    global_gc_for_each_objspace(vm, local_gate, gc_marks_prepare_full);
    global_gc_for_each_objspace(vm, local_gate, gc_marks_start_full);

    rb_vm_ractor_mark(vm);

    global_gc_for_each_objspace(vm, local_gate, gc_marks_rest_no_finish);
    global_gc_for_each_objspace(vm, local_gate, gc_marks_finish);
	...
	ccan_list_for_each(&GET_VM()->objspace_set, local_gate, gate_node) {
	    shareable_total += local_gate->global_shareable_object_estimate;
	}
	unlock_ractor_set();
	objspace_coordinator->rglobalgc.global_gc_threshold = (size_t)(shareable_total * gc_params.sharedobject_limit_factor);
    }

#if RGENGC_PROFILE > 0
    global_gc_for_each_objspace(vm, local_gate, gc_record_old_objects);
#endif

    global_gc_for_each_objspace(vm, local_gate, gc_prof_mark_timer_stop);
    global_gc_for_each_objspace(vm, local_gate, gc_sweep);
}
...
{
    VM_ASSERT(GET_OBJSPACE_OF_VALUE(a) == objspace);
    VM_ASSERT(GET_OBJSPACE_OF_VALUE(b) == objspace);

    if (!RVALUE_OLD_P(objspace, a) || RVALUE_OLD_P(objspace, b)) return;
...
static void
gc_writebarrier_incremental_remote_parent(VALUE a, VALUE b, rb_objspace_t *objspace)
{
    gc_report(2, objspace, "gc_writebarrier_incremental_remote_parent: [LG] %p -> %s\n", (void *)a, rb_obj_info(b));
...
void
rb_gc_impl_writebarrier_single_objspace(void *objspace_ptr, VALUE a, VALUE b)
{
    rb_objspace_t *objspace = objspace_ptr;
    VM_ASSERT(GET_OBJSPACE_OF_VALUE(b) == objspace);
...
void
rb_gc_impl_writebarrier_gc_blocked(void *objspace_ptr, VALUE a, VALUE b)
{
    rb_objspace_t *objspace = objspace_ptr;
    VM_ASSERT(GET_OBJSPACE_OF_VALUE(b) == objspace);
    ...
    if (GET_OBJSPACE_OF_VALUE(a) == objspace) {
	if (is_incremental_marking(objspace)) {
	    gc_writebarrier_incremental(a, b, objspace);
	}
	else {
	...
	if (is_incremental_marking(objspace)) {
	    gc_writebarrier_incremental_remote_parent(a, b, objspace);
	}
    }
}
...
garbage_collect_global(rb_objspace_t *objspace, unsigned int reason, bool need_finalize_deferred)
{
	...
	ccan_list_for_each(&vm->objspace_set, local_gate, gate_node) {
	    ...
	    global_gc_possible = global_gc_possible && (rb_darray_size(objspace->heap_pages.sorted) && ((reason & GPR_FLAG_METHOD) || ready_to_gc(os)));
...
{
    VM_ASSERT(objspace->local_gate->local_gc_level > 0 || objspace->local_gate->running_global_gc);
    VM_ASSERT(!GET_RACTOR()->teardown_cleanup_done);
    ...
    if (global_gc) {
	VM_ASSERT(objspace->local_gate->running_global_gc);
	ASSERT_vm_locking();
...
void
rb_gc_impl_start(void *objspace_ptr, bool full_mark, bool immediate_mark, bool immediate_sweep, bool global, bool compact)
{

    if (global) {
        if (!full_mark)       rb_raise(rb_eArgError, "`full_mark' must be true if `global' is true");
        if (!immediate_mark)  rb_raise(rb_eArgError, "`immediate_mark' must be true if `global' is true");
        if (!immediate_sweep) rb_raise(rb_eArgError, "`immediate_sweep' must be true if `global' is true");
	if (compact)          rb_raise(rb_eArgError, "global compaction is not yet implemented");
    }
    ...
    if (!global) reason &= ~GPR_FLAG_GLOBAL;

    if(!gc_deactivated(rb_get_objspace_coordinator())) {
...
gc_move(rb_objspace_t *objspace, VALUE src, VALUE dest, size_t src_slot_size, size_t slot_size)
{
    ...
    if (mutable_shareable_permission) {
        MARK_IN_BITMAP(GET_HEAP_MUTABLE_SHAREABLE_PERMISSION_BITS(dest), dest);
    }
    else {
...
gc_sort_heap_by_compare_func(rb_objspace_t *objspace, gc_compact_compare_func compare_func)
{
    for (int j = 0; j < HEAP_COUNT; j++) {
        ...
        /* Sort the heap so "filled pages" are first. `heap_append_added_page` adds to the
...
bool
rb_gc_impl_during_local_gc_p(void *objspace_ptr)
{
    rb_objspace_t *objspace = objspace_ptr;
...
bool
rb_gc_impl_using_local_limits(void *objspace_ptr)
{
    rb_objspace_t *objspace = objspace_ptr;

    return using_local_limits(objspace);
}
...
rb_gc_impl_stat_heap(void *objspace_ptr, VALUE heap_name, VALUE hash_or_sym)
{
	...
	if (!during_gc) rb_borrowing_sync_unlock(objspace->ractor);
        ...
        if (SYMBOL_P(hash_or_sym)) {
            return stat_one_heap(heap, Qnil, hash_or_sym);
        }
...
gc_compact(VALUE self)
{
    ...
    rb_gc_impl_start(rb_gc_get_objspace(), true, true, true, false, true);
    gc_config_full_mark_set(full_marking_p);
...
static void
absorb_page_into_objspace(rb_objspace_t *objspace, struct heap_page *page, int heap_idx)
{
    VM_ASSERT (page->objspace != objspace);
    VM_ASSERT(objspace->local_gate->currently_absorbing);

    ccan_list_del_init(&page->page_node);

    rb_heap_t *heap = &heaps[heap_idx];

    VM_ASSERT(page->slot_size == heap->slot_size);
    page->heap = heap;
    heap->total_allocated_pages++;
...
static void
heap_append_free_page(rb_heap_t *heap, struct heap_page *page)
{
    struct heap_page *end_page = heap->free_pages;
    if (end_page) {
	while (end_page->free_next) end_page = end_page->free_next;
	end_page->free_next = page;
    }
    else {
	heap->free_pages = page;
    }
...
static void
merge_freepages(rb_objspace_t *receiving_objspace, rb_objspace_t *closing_objspace, int heap_idx)
{
    rb_objspace_t *objspace = closing_objspace;
    rb_heap_t *closing_heap = &heaps[heap_idx];
    objspace = receiving_objspace;
    rb_heap_t *receiving_heap = &heaps[heap_idx];
    
    heap_append_free_page(receiving_heap, closing_heap->free_pages);
}

static void
transfer_heap(rb_objspace_t *receiving_objspace, rb_objspace_t *closing_objspace, int heap_idx)
{
    rb_objspace_t *objspace = closing_objspace;
    rb_heap_t *heap = &heaps[heap_idx];
    struct heap_page *page = ccan_list_top(&heap->pages, struct heap_page, page_node);
    while (page) {
	absorb_page_into_objspace(receiving_objspace, page, heap_idx);
	page = ccan_list_top(&heap->pages, struct heap_page, page_node);
    }
    merge_freepages(receiving_objspace, closing_objspace, heap_idx);
}

static void
transfer_all_heaps(rb_objspace_t *receiving_objspace, rb_objspace_t *closing_objspace)
{
    for (int i = 0; i < HEAP_COUNT; i++) {
	transfer_heap(receiving_objspace, closing_objspace, i);
    }
}

static void
update_heap_counts(rb_objspace_t *objspace_to_update, rb_objspace_t *objspace_to_copy_from, int heap_idx)
{
    rb_objspace_t *objspace = objspace_to_copy_from;
    rb_heap_t *heap_to_copy_from = &heaps[heap_idx];

    size_t total_allocated_pages = heap_to_copy_from->total_allocated_pages;
    size_t force_major_gc_count = heap_to_copy_from->force_major_gc_count;
    size_t force_incremental_marking_finish_count = heap_to_copy_from->force_incremental_marking_finish_count;
    size_t total_allocated_objects = heap_to_copy_from->total_allocated_objects + heap_to_copy_from->newly_created_by_borrowing_count;
    size_t total_freed_objects = heap_to_copy_from->total_freed_objects;
    size_t final_slots_count = heap_to_copy_from->final_slots_count;

    objspace = objspace_to_update;
    rb_heap_t *heap_to_update = &heaps[heap_idx];

    heap_to_update->total_allocated_pages += total_allocated_pages;
    heap_to_update->force_major_gc_count += force_major_gc_count;
    heap_to_update->force_incremental_marking_finish_count += force_incremental_marking_finish_count;
    heap_to_update->total_allocated_objects += total_allocated_objects;
    heap_to_update->total_freed_objects += total_freed_objects;
    heap_to_update->final_slots_count += final_slots_count;
}
...
update_objspace_counts(rb_objspace_t *objspace_to_update, rb_objspace_t *objspace_to_copy_from)
{
    ...
    for (int i = 0; i < HEAP_COUNT; i++) {
	update_heap_counts(objspace_to_update, objspace_to_copy_from, i);
    }
}

static void
absorb_fin_tbl(rb_objspace_t *objspace_to_update, rb_objspace_t *objspace_to_copy_from)
{
    rb_objspace_t *objspace = objspace_to_update;
    st_table *table1 = finalizer_table;

    objspace = objspace_to_copy_from;
    st_table *table2 = finalizer_table;

    absorb_table_contents(table1, table2);
}

static void
absorb_obj_id_tbls(rb_objspace_t *objspace_to_update, rb_objspace_t *objspace_to_copy_from)
{
...
static void
absorb_empty_pages(rb_objspace_t *objspace_to_update, rb_objspace_t *objspace_to_copy_from)
{
    ...
    if (prev_page) {
	prev_page->free_next = objspace_to_update->empty_pages;
	objspace_to_update->empty_pages = objspace_to_copy_from->empty_pages;
	objspace_to_update->empty_pages_count += objspace_to_copy_from->empty_pages_count;
    }

}

void
rb_gc_impl_absorb_contents(void *receiving_objspace_ptr, void *closing_objspace_ptr)
{
    rb_objspace_t *receiving_objspace = receiving_objspace_ptr;
    rb_objspace_t *closing_objspace = closing_objspace_ptr;

    absorb_fin_tbl(receiving_objspace, closing_objspace);
    absorb_obj_id_tbls(receiving_objspace, closing_objspace);

    transfer_all_heaps(receiving_objspace, closing_objspace);
    ...
    gc_ractor_newobj_cache_clear(closing_objspace->ractor->newobj_cache, NULL);
    gc_ractor_newobj_cache_clear(closing_objspace->ractor->newobj_borrowing_cache, NULL);
    closing_objspace->ractor->newobj_cache = NULL;
    closing_objspace->ractor->newobj_borrowing_cache = NULL;
...
void
rb_gc_impl_attach_local_gate(void *objspace_ptr, void *local_gate_ptr)
{
    rb_objspace_t *objspace = objspace_ptr;
    rb_objspace_gate_t *local_gate = local_gate_ptr;

    objspace->local_gate = local_gate;

    if (objspace->ractor == ruby_single_main_ractor) {
	objspace->local_gate->belong_to_single_main_ractor = true;
    }
}
